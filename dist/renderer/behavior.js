export class BehaviorPlanner {
    ...
    curiosity = 0.5;
    // Timing/scheduling
    nextIdleActionTime = 0;
    actionCooldown = 0;
    // Custom bounce physics
    isBouncing = false;
    bounceTime = 0;
    constructor(rig, observer) {
        this.rig = rig;
    ...
    updateDebugOverlay(profile) {
        ...
        if (elState)
            elState.innerText = this.currentState;
        if (elGaze)
            elGaze.innerText = `${this.rig.params.gazeX.toFixed(2)} / ${this.rig.params.gazeY.toFixed(2)}`;
        if (elHead)
            elHead.innerText = `${this.rig.params.headAngle.toFixed(1)}°`;
        if (elBody)
            elBody.innerText = `${this.rig.params.bodyLean.toFixed(1)}°`;
        if (elActivity)
            elActivity.innerText = profile.activityIntensity.toFixed(2);
        if (elIdle)
            elIdle.innerText = `${profile.idleTimeSeconds.toFixed(1)}s`;
        if (elEnergy)
            elEnergy.innerText = this.energy.toFixed(2);
        if (elSleep)
            elSleep.innerText = this.sleepiness.toFixed(2);
    }
}
